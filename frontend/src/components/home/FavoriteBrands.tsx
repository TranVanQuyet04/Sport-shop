...
const FavoriteBrands = () => {
  ...
  return (
    <section className="ui-panel overflow-hidden rounded-lg p-5 sm:p-8">
      <div className="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="w-fit rounded-full border border-red-100 bg-red-50 px-4 py-2 text-sm font-bold text-red-700">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {isLoading ? (
          ...
          brands.slice(0, 4).map((brand) => (
            <div
              key={brand.id}
              className="group relative min-h-[250px] overflow-hidden rounded-lg bg-zinc-950 shadow-sm"
            >
              <img
                src={
                  brand.logo ||
              ...
              <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-5">
                ...
                <button
                  ...
                  className="inline-flex h-10 shrink-0 items-center gap-2 rounded-sm bg-white px-4 text-sm font-black uppercase text-black transition hover:bg-red-600 hover:text-white"
                >
                  Shop
